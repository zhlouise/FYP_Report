...
\begin{table}[ht]
    ...
    \begin{itemize}[nosep, leftmargin=*, label=\textbullet]
        ...
        \item RRobust in dynamic, uncertain environments.
...
values was the most suitable approach for our case. Below is a brief introduction to LightGBM model.
\paragraph{LightGBM Model} ~\\
...
however it addresses some of the limitations of these models in terms of effieciency 
and scalability for large datasets with high dimensional feautures \cite{ke2017lightgbm}.
...
optimizes boosting and leads to faster congergence when combined with gradient based sampling. 
...
and in the ground controller. For this experiment, we use QGround Control \cite{QGroundControl_PlanView} as the
ground controlling software and use 433MHz Radio Telemetry for communication
between Pixhawk and QGround Control. 
...
landing. Figure \ref{fig:qgc_plan_view} shows a screenshot of the QGC plan view, which specifies the
different toolbars, including plan tools, the command editor, and the mission
list.
\begin{figure}[H]
    \centering
...
editing the mission command list. For instance, if our experiment is hovering
over a tree, we will assign a speed of 2 m/s, hold for 300 seconds, and the
altitude will be roughly around 30m. Figure \ref{fig:qgc_mission_list} shows an example of a mission
command list in QGC.
\begin{figure}[H]
    \centering
...
as we conducted test runs using both a locally developed A* algorithm and
waypoint navigation in QGroundControl. 
...
in QGround Control. Then, we upload the coordinates and the data to the Pixhawk
...
package developed by the Autonomous Systems Lab at ETH Zurich. The experiments
...
At first, we specified the map as a polygon in the GUI. Once we input the
polygon, the algorithm will determine the coordinates of the polygon, the sweep
distance, and the altitude.  An example of result interpretation for a polygon
shown in Figure \ref{fig:cpp_gui} is as follows: 
\begin{figure}[H]
    \centering
...
high in altitude. This simulates a camera/sensor flying at 3m height with a
field of view. The sweeps are spaced 1.07177m apart to ensure 60 percent overlap
between consecutive passes. The Boustrophedon algorithm divides your polygon
...
\paragraph{Covergae Path Planning in QGround Control} ~\\
...
In summary, we conducted waypoint-based navigation experiments primarily to
...
trees. 
Additionally, we implemented coverage path planning to obtain dynamic UAV
movement data, which enables the development of LiDAR SLAM in our system. LiDAR
SLAM is crucial in our system to cross-validate the data obtained through GNSS-R
...
FFZs from \hl{experiment (a) and (b)} are plotted onto a satellite image map,
with colors representing the ratio of $\frac{C/N_{0, reflected
signal}}{C/N_{0,direct signal}}$.
\begin{figure}[H]
    \centering
...
signals repectively are misclassified as tree reflected signals. This could be due to the similarity in the features of these
...
This results are acceptable as the model will later be usef to identify vegetation reflected signals mostly.
...
binary classification model at various threshold settings. It plots plots the True Positive Rate (TPR) against the False Positive
