...
conventional sensing methods often face the limitations of high costs, weather
...
vegetation, the remote sensing methods could be classified into two categories
based on the distance between the target object and the measurement sensor: 1)
...
\begin{enumerate}
\item To introduce an autonomous path planning framework onboard the UAV
platform for optimized GNSS-R remote sensing. The framework aims to identify
potential vegetation areas and generate accurate trajectories for navigation
during data collection. Consequently, the proposed system enables the UAV to
autonomously acquire high-quality GNSS raw data for environmental monitoring.
\item To analyze and model the correlations between signal propagation parameters
retrieved through GNSS-R and ground vegetation conditions. This involves the collection
and post-processing of raw GNSS data, extraction of GNSS signal measurements, and the
analysis of GNSS signal parameters with respect to the ground region reflecting the
signal. Ultimately, such correlations would be modelled both quantitatively and 
qualitatively such that the ground vegetation conditions could be effectively predicted
from the GNSS signal properties received onboard the UAV.
\item To classify signals reflected by vegetation and predict vegetation parameters
based on raw GNSS data using a machine-learning-based approach. The classification 
task will help to identify tree reflected signals which will be sent to GNSS-R framework, 
while the regression task will help to predict vegetation parameters and validate GNSS-R framework. 
Classification part will be mainly based on gradient boosting and
decision tree based models while the regression part will make use of advanced neural
network architectures. Ultimately, the machine learning part will complement the GNSS-R
framework to enhance the robustness of vegetation parameter prediction.
...
This involves creating a modular SLAM system, incorporating LiDAR odometry as the 
front-end and GPS-coupled pose graph optimization SLAM as the back-end. As a result, 
an accurate vegetation map could be generated to complement the GNSS-R vegetation 
monitoring system.
\end{enumerate}
...
suitable algorithms for autonomously navigating an UAV in vegetated environments.
...
jagged paths often produced by standard wavefront algorithms. The section below
reviews existing GNSS-R techniques that can be used once the path planning
methodologies have been established.


\subsection{GNSS-R}
The concept of using GNSS-R for remote sensing was first proposed in 1993, where
the correlation between direct and scattered GNSS signals was proven promising
for the application of ocean altimetry \cite{MartnNeira1993APR}. Today, apart
from the originally proposed application, GNSS-R techniques have been extensively
...
modeled, where simulations revealed that tree trunk scattering effects would
...
\cite{RodriguezAlvarez2011}. Ultimately, an RMSE of 3-5 cm in estimating the
...
\begin{table}[H]
    ...
    monitoring. \\
    \midrule
    Rodríguez-Álvarez et al. (2011) & A SMIGOL reflectometer, which leverages GNSS signal
    interference patterns to measure soil and vegetation conditions, is used to achieve
    3-5 cm RMSE in estimating simple vegetation height. \\
    \midrule
...
platforms - such as a UAV - remains underexplored. Therefore, it is critical to
develop a systematic framework for a UAV-based GNSS-R system, in which
...
approaches, further enhancing the system robustness. The section below reviews
existing machine learning-based techniques that are relevant to this project.
...
Apart from GNSS-R and ML, the project will also integrate LiDAR SLAM to provide
spatial reference for the GNSS-R derived vegetation parameters. The section
below reviews existing LiDAR SLAM techniques relevant to this project.
...
SLAM works. The landmarks are connected by red lines to describe the
correlations between landmarks. As the robot passes a landmark more than once,
these correlations increase, which makes the red spring thicker
...
% --- Methodology ---
\section{Methodology}
In this section, the UAV platform used for this project and the system architecture
design is presented. With the system dividing into four key workflows of path planning,
GNSS-R processing, machine learning, and LiDAR SLAM, a detailed overview of the processes
and mechanism adopted for each workflow will also be discussed.
...
downward-facing RHCP antenna (nadir) located at the bottom. As a result, both
...
architecture: path planning, GNSS-R, machine learning, and LiDAR SLAM, as illustrated
in Figure \ref{System Architecture}. Here, the path planning processes are indicated
in green, GNSS-R processes in blue, machine learning processes in yellow, and
LiDAR SLAM processes in orange.
...
satellite imagery will be used to identify vegetated areas to achieve autonomous
mission designs. With the planned trajectory designed, the ground control
station will facilitate the mission execution to enable the collection of both
GNSS raw data and LiDAR point cloud data. Then, both the zenith and nadir GNSS
receiver's data will be post-processed into GNSS measurements, where they will
be analyzed with respect to the Fresnel zones of ground reflections, while any
correlations between the GNSS measurements and ground vegetation parameters will
be systematically modelled. On the other hand, machine learning methods will
also be adopted to process the collected GNSS data. A trained LightGBM model
will be used to classify the GNSS signals according to the signal reception
scenarios: direct open sky reception, signals reflected from vegetation, and
signals reflected from bare ground. The classification results will then be
integrated with the GNSS-R framework for more accurate signal analysis, while
regression models will also be trained for vegetation parameter prediction.
Meanwhile, the LiDAR data will be synchronized with the timestamps obtained from
the flight controller's GPS module. Then, both LiDAR and GPS data will be
...
front-end and pose graph optimization as the back-end. Together, the 3D
vegetation map will be used to cross-validate any vegetation parameters
retrieved by GNSS-R models and regression results, enabling a robust system for
...
measures the difference between red light, which is absorbed by trees, and
near-infrared light, which is strongly reflected by vegetation
\cite{GISGeography_NDVI}. Below is the equation to calculate NDVI:
\begin{equation}
...
algorithm are discussed in the results and discussion section. The section below 
provides a detailed overview of the GNSS-R framework used for data processing once 
the path planning is completed and the UAV is in operation.
...
to the noise power per unit bandwidth, usually with a unit of decibel-Hertz (dB-Hz)
...
in dBm or dBW; and $BW$ refers to the bandwidth of the signal in Hz.
...
\end{equation}
where $R^i$ refers to the true distance between the satellite and the receiver;
...
\begin{figure}[H]
    ...
    \caption{Schematic diagram of the local coordinate axes defined.}
\end{figure}
...
One key issue for the GNSS-R is not being able to classify the received signals
into direct signals and reflected signals. Therefore, in this project, machine
learning techniques are employed to classify the received signals based on the
extracted GNSS measurements, as outlined in the section below.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Included_Images/LossCurve.png}
    \caption{Training and Validation Loss Curves for LightGBM.}
\end{figure}
From the figure above, it is clear that the model did not overfit as the loss curves for 
training and validation are close to each other and decrease steadily. Further performance assessment results 
of the model are presented in the results and discussion section. The section below provides a detailed overview of the LiDAR SLAM framework used for
...
$k$ evolved from the prior state at time $k-1$, and it is shown as follows:
\begin{equation}
...
vector. We can approximate $w_{k-1}$ as $N(0,Q_k)$ where it has a zero-mean
Gaussian distribution with covariance matrix $Q_k$.
...
$x_{k-1}$, and $Q$ is the noise covariance matrix that describes the
...
predicted measurement at the current iterative estimate $K$:
\begin{equation}
...
accuracy. This technique is rooted in factor graph optimization algorithm where
Maximum A Posteriori (MAP) is also used to factorize the the collective
posterior distribution of the trajectory state X as shown
\cite{wen2021factor}:
...
expansion at the current estimate $\mathcal{X}^{(i)}$ where the state is
...
/cloud\_registered\_body from /laserMapping, and GPS and IMU information from
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{Included_Images/qgc_mission_list.jpg}
...
In this section, the results obtained from the four different implementations of
this project will be presented, which includes path planning, GNSS-R signal
analysis, machine learning classification, and LiDAR SLAM. The results are
analyzed to evaluate the effectiveness of each subsystem, and discuss how
autonomous navigation enables reliable data collection, how GNSS-R measurements
vary across different surfaces, and how machine learning and SLAM enhance signal
interpretation and spatial consistency.
...
selection have been explained in the methodology section. The performance of the model is
evaluated based on various metrics including F1-score, confusion matrix and Receiver Operating Characteristic (ROC) curve. The subsections
...
A confusion matrix is a technique to visualize the performance of a classification model on a set of test data. 
The diagonal elements of the matrix represent correctly classified samples,
while the off-diagonal values indicate misclassified ones. The confusion matrix for the LightGBM signal classification 
...
A ROC curve is a technique to graphically represent the performance of a 
binary classification model at various threshold settings. It generally plots the True Positive Rate (TPR) against the False Positive
...
\subsection{Case Study: Experiment (i)}
...
polarization. To solve this problem, robust signal classification is required to
...
In this section, the proposed solutions to the limitations of each component at
its current stage will be presented. The frameworks discussed include path
planning, GNSS-R, machine learning, and LiDAR SLAM. The discussed solutions will
be executed in the next stage of our project. 
...
results in UAV simulation and the real world. By simulating flight trajectories,
we can verify our locally developed algorithms prior to real-world application. 

To address this limitation, we will use UAV Package Delivery
\cite{MathWorks_UAV_Delivery}, developed by MATLAB, for small quadrotors, and
Microsoft AirSim \cite{shah2017airsimhighfidelityvisualphysical} to conduct
advanced simulations. These simulation software packages are built specifically
for unmanned aerial vehicles; therefore, they provide the necessary aerodynamics
and real-world physics packages to minimize the gap between real-world and
simulation environments. 

Currently, we do not deploy our algorithms in the real-world UAV; instead, we
use the control and path planning system in QGroundControl to communicate with
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Included_Images/indoor_experiment.png}
...
potential vegetation areas. In the future, we plan to integrate LiDAR SLAM data
into our path planning pipeline to achieve a simultaneous mapping system. 
...
for FFZs covering the trees will be explored. Lastly, a model of identified correlations
between GNSS measurements and ground features will be summarized to achieve the
ultimate objective of this project.


\subsection{Machine Learning}
The next part of this project will focus on developing a neural-network-based
model to perform regression tasks to predict vegetation parameters such as
biomass, VWC, tree height, etc. Literature shows that neural networks have been
effective in performing such regression tasks using features such as Delay
Doppler Maps (DDMs), signal power, and polarization parameters, etc.
\cite{hu2019detecting,pilikos2024biomass}. Simultaneously, the dataset for the
LightGBM model will be further expanded by collecting more GNSS-R data in
controlled environments to improve classification performance.
...
Through processing the data collected during the first half of this project, one
...
Discussion section of the report. Particularly, this is due to the fact that not
