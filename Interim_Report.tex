...
\begin{enumerate}
    ...
    obstacles.
    \item Initialize the heapfile to store the exploring nodes, check the node with lowest \texttt{f\_score}, skip if already explored.
    ...
    update the parent node, adjust the \texttt{g\_score} to the lowest cost, and add to
...
red light (which vegetation absorbs). Below is the equation to calculate NDVI:
\begin{equation}
...
areas and obtaining dynamic data for LiDAR SLAM. In this research, the CPP
algorithm that will be implemented is called Boustrophedon Cell Decomposition.
The Boustrophedon Cell Decomposition algorithm works by decomposing the map into
cells based on the polygonal shape of the map and the obstacles present. After
decomposing the map into cells, the algorithm will then generate a sweeping path
in each cell based on the given parameters \ref{fig:boustrophedon}. Having this
sweeping technique in all the cells will ensure that the entire area is covered.
Below is the detail explanation of now the boustrophedon cell decomposition is
implemented, using the framework developed by Autonomous Systems Lab at ETH
Zurich \cite{B_hnemann_2021}.
...
Figure \ref{fig:in_event} illustrates an example of an IN event:
    ...
    sweep line intersection. OUT event moves the intersecting vertexes as the
    opposite direction as the sweep line.
...
Figure \ref{fig:out_event} illustrates an example of an OUT event:
    \begin{figure}[H]   
        ...
        \caption{Boustrophedon Cell Decomposition OUT Event Example}
        \label{fig:out_event}
        \end{figure}
\end{itemize}
