...
localization. As a result, our research provides a unique solution by enhancing
the UAV and GNSS-R combination with the integration of autonomous navigation. 
...
intelligence, and control systems. In robotics, path planning is a technique that
is used in navigating a robot autonomously from an origin to a destination via a
collision-free route. Path planning is primarily a geometric problem that
generates a path according to a given sequence of waypoints, without accounting
for variations over time. In contrast, trajectory planning defines the path as a
function of time, assigning specific timing to the geometric route
...
outdoor environments. Consequently, we prioritize path planning methodologies in
this work, reserving trajectory planning and optimization for future
investigation.

The earliest sensor-based, obstacle-avoiding path planning algorithms are known
as bug1 and bug2 algorithms \cite{choset2005principles}. These algorithms are
designed to first navigate around the an obstacle and then determine the
shortest path around the obstacle to reach the goal (shown in Figure \ref{fig:bug_algorithms}).
\begin{figure}[H]
    \centering
...
guarantee the optimal path. With the rise of SLAM (Simultaneous Localization and
Mapping) techniques, we are able to create maps of the navigation environment,
which enabled the transition to use map-based path planning methods. Therefore,
in our system, we are using commonly found grid-based path planning algorithms,
which enables us to create an intelligent UAV system that can identify potential
vegetation areas using GNSS-R. 
...
planning mechanisms, we conducted our literature review across three different
...
and examines the shortest path by comparing nodes in the window
...
regions, called cells \cite{gasparetto2015path}. The decomposition process can be explained as follows:
subdivision of space into numbered polygons, creation of a connectivity graph,
identification of regions to be crossed, and generation of the path.  The figure
\ref{fig:cell_decomposition} shows the detailed steps of the exact cell
decomposition process. In our system, we utilize exact cell decomposition,
specifically boustrophedon decomposition, for coverage path planning. 
\begin{figure}[H]
    \centering
...
while covering the area. Similarly, in wavefront algorithms, having zig-zag
...
surveying the dense canopy to obtain dynamic data for LiDAR SLAM.
...
vegetation environments and identify potential Fresnel Zones, we primarily need
to fly in outdoor, high-altitude, open spaces. To determine the optimal
algorithm for building an intelligent UAV system, we conducted a baseline
comparison among the Dijkstra algorithm, the A* algorithm, and the RRT
algorithm.
...
algorithm is suitable for offline path planning. Therefore, our system is
...
mapping approaches such as NavMeshes are widely used in 3D simulation software.
...
In our research, we will focus on regular grids that connect nodes across 26
...
Our system utilizes a grid map defined in 3D space, featuring 26-connectivity,
...
grid with 8-connectivity. We first convert the coordinates of the given map into
...
To evaluate the algorithm's performance, we conducted a series of test runs
using vegetation density maps in different categories. The maps were obtained
using QGround Control software from satellite imagery and then imported into an
...
areas and for obtaining dynamic data for LiDAR SLAM. In this research, we
implemented Boustrophedon Cell Decomposition as the CPP algorithm. The
...
algorithm are discussed in the discussion section.
...
\begin{itemize}
    ...
    \item\textbf{Carrier Phase}: Carrier phase refers to the precise range observables that are derived 
     ...
     exhibit 2.5 times broader carrier phase spread compared to thos reflected from bare 
...
Data was collected from muliple locations with different environmental settings, in order to have 
a diverese dataset that captures the three classes of signals. The scenariors include:
...
For the classification task, a LightGBM model was selected due to its advantages with native 
support for NaN values. This feature of the model is particularly important for our case as 
raw GNSS data often contains missing values due to signal blocckages or reveiver limitations. Cleaning the
data by removing the NaN values would lead to sinificant loss of data, as cleaning process would have to be carried
out either row wise of column wise. Replacing the NaN values with zeros or mean values would introduce bias to the dataset, 
which would potentially affect the performance of the model. Therefore, using a model with native support for NaN 
values was the most suitable approach for our case. 
...
\subsection{GNSS-R}
After the successful execution of the path planning algorithms onboard the UAV, the GNSS
measurements from both the zenith and nadir antenna are recorded and post-processed. 
Particularly, the ratio of the nadir $C/N_0$ and the zenith $C/N_0$ are analyzed with 
respect to the FFZ locations. Here, the FFZs from \hl{experiment (a) and (b)} are plotted
onto the satellite image map, with the colors representing the ratio of 
$\frac{C/N_{0, reflected signal}}{C/N_{0,direct signal}}$.
\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
...
\end{figure}
From the above, it could be seen that, when the FFZ is over water/bare ground, the
$C/N_0$ ratio between the reflected signal and the direct signal tends to be higher. 
This indicates stronger reflections tends to occur when the FFZ is over water or bare
ground compared to when the FFZ is over trees.

Yet, when the UAV hovers over trees, more reflected signals could be observed, as shown in
Figure \ref{Pr_Error Distribution} below. By definition, since the majority of pseudorange
error is contributed by signal reflections, the signals received under a non-obstructed open
sky environment should have a near-zero mean pseudorange error. However, while the zenith 
antenna is pointed toward the open sky, the pseudorange error distribution displays a small
bump centered at large positive psuedorange errors, indicating that a signal reflected from
the ground is received. Moreover, this "bump" is more significant on plot (b), indicating 
more ground reflections observed. 
\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
...
While the above analysis presents key differences in the GNSS measurements that
could be used to distinguish tree-reflected signals thus be able to used for
identifying vegetated areas, it also presents a crucial assumption in this study
that is necessary to be eliminated. Till this point, it was assumed that the zenith
antenna only receives the direct signal while the nadir antenna only receives the 
reflected signal. However, as shown in Figure \ref{Pr_Error Distribution}, this is not
the case. To further investigate this, the distribution of difference between the absolute
value of nadir pseudorange error and zenith pseudorange error is plotted in Figure
...
\end{figure}
First, since many observations has a near-zero
difference between nadir and zenith antenna pseudorange error, the nadir antenna pseudorange
error have similar values compared to the zenith antenna pseudorange error in such observations.
To achieve this, two possible scenarios could occur: 
\begin{enumerate}
    \item The zenith antenna receives ground-reflected signals, which has a
    large pseudorange error comparable to that of the nadir antenna. This
    further validates the previous discussion on Figure \ref{Pr_Error
    Distribution}.
    \item The nadir antenna receives direct propagated signals, which has a small pseudorange
    error comparable to that of the zenith antenna. The possibility of this happening is
    also validated in Figure \ref{Pr_Error Distribution}, where near-zero values of pseudorange
    error exists in the distributions for nadir antenna.
\end{enumerate}
Furthermore, Figure \ref{Pr_Error Difference} shows the existence of
observations where the zenith pseudorange error is much greater than the nadir
pseudorange error, indicating the reception of reflected signals by the zenith
antenna. Since the zenith antenna is a RHCP antenna and the nadir antenna is an 
LHCP antenna, this also indicates that a GNSS RHCP antenna is able to receive
LHCP signals and vice versa. To further investigate this observation, both antenna
are tested in an open sky environment and their respective received carrier to noise 
ratio are plotted into distribution curves as shown in Figure \ref{Antenna Testing Open Sky}.
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.49\textwidth}
...
\end{figure}
Assuming all signals received under the open sky environment is non-reflected thus RHCP,
it is clear that the LHCP antenna (in orange) can indeed receive RHCP signals, just at a 
lower $C/N_0$ compared to the RHCP antenna due to lower gains. This phenomenon is more 
pronounced in a further experiment, where both antenna were being lifted in the air. In the 
first five minutes, both antenna face upward; in the last five minutes, both antenna face 
downward. The corresponding received $C/N_0$s are plotted against observation time (epochs)
in the figure below.
\begin{figure}[H]
    \centering
...
\end{figure}
The above observations could be qualitatively summarized in the table below:
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|}
    \hline
...
\end{table}
Since both direct signals and reflected signals can be received by both the zenith RHCP 
antenna and the nadir LHCP antenna, and that their signal properties differ when received
by different antenna, it is crucial to classify them to distinguish the reflected signals
from the direct signals to facilitate a more accurate GNSS-R framework. This could be easily
achieved through machine learning approaches, which will be discussed in detail in the next
section.   
