...
collecting data and testing developed methodologies. To achieve the tasks, each
members are mostly working individually with the task allocation shown below: 
\begin{itemize}
    \item Path planning: GAMAGE Sashenka (Sashenka)
    \item GNSS-R Analysis : ZHOU Jiayi (Louise)
    \item Machiene Learning: MAHMUD Md Sahat (Sahat)
    \item LiDAR SLAM: Tan Qing Lin (Giselle)
\end{itemize}

Although each member focuses mainly on their individual area, some
responsibilities are allocated to ensure efficient progress across all aspects
of the project. With that, Sashenka handles hardware integration for flight
control and executes autonomous flight missions using QGroundControl. Louise is
responsible for leading the project, coordinating data collection experiments,
and managing task distribution. Sahat undertakes the licensing process required
to operate the UAV and conducts manual flight experiments. Giselle is
responsible for purchasing logistics and the design and manufacture of
3D-printed components for the UAV.
...
collaboration is expected in some key areas. For instance, Louise and Sahat will
work more closely on GNSS data classification, while Sashenka and Giselle will
collaborate on integrating the SLAM system into the path planning framework.
