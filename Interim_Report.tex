...
\cite{mailka2024efficient, chang2019gnss} single-sensor SLAM systems (e.g.
...
\subsection{Machine Learning}
The next part of this project will focus on developing a neural-network-based model
to perform regression tasks to predict vegetation parameters such as biomass, VWC,
tree height, etc. Literature shows that neural networks have been effective in performing
such regression tasks using features such as Delay Doppler Maps (DDMs), signal power,
and polarization parameters, etc. \cite{hu2019detecting,pilikos2024biomass}. Simultaneously,
the dataset for the LightGBM model will be further expanded by collecting more GNSS-R data in controlled
environments to improve classification performance.
...
validate methodologies and writing final project report. 

The path planning part of the project is lead by GAMAGE Sashenka, who has been focusing 
on developing and testing the path planning algorithms in local mapping environment and
QGroundControl. So far, the path planning part has been progressing as per the timeline in the Gantt chart and 
future works mainly focus on conducting simulations in dynamic UAV environment and real world UAV testing. 

The GNSS-R part of the project is lead by ZHOU Jiayi, who has been focusing on conducting GNSS-R experiments
and analyzing the collected data. So far, the GNSS-R part has been progressing as per the timeline in the Gantt chart and
future works mainly focus on integrating machine learning classification results into the GNSS-R pipeline
to classify between reflected and direct signals and exploring the sensitivity of GNSS carrier phase measurements
for FFZs covering the trees.

The ML part of the project is lead by MAHMUD Md Sahat, who has been working on developing a 
classification model using LightGBM to identify true tree reflected signals from the collected GNSS-R data.
So far, the ML part has been progressing as per the timeline in the Gantt chart and
future works will be focusing on developing neural netwok based model to perform regression tasks to predic 
vegetation parameters. Sahat has also been responsible for piloting the drone during the experiments.

The LiDAR SLAM part of the project is lead by TAN Qing Lin. She has been developing a modular LiDAR SLAM 
framework which she has tested in both with and without GPS integration. So farm the LiDAR SLAM part has been
progressing as per the timeline in the Gantt chart and future works mainly focus on solving the point cloud
alignment issues after GPS integration and fine-tuning the modular SLAM framework to improve robustness.
