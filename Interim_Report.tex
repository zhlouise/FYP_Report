...
points in the graph \cite{javaid2013understanding}. However, finding the shortest
path across all the points in the map incurs high computational cost.
Therefore, it is impractical to use for real-time UAV path planning. 
...
it does not guarantee the shortest path. This was proven by Karaman and
Frazzoli \cite{karaman2010incrementalsamplingbasedalgorithmsoptimal}, showing
that the probability of the RRT algorithm converging to an optimal solution is
zero. Therefore, the RRT*, an optimal version of the RRT algorithm, was
proposed. 
\begin{figure}[H]
    \centering
...
between landmarks. As the robot passes a landmark more than once, these
...
Figure \ref{fig:out_event} illustrates an example of an OUT event:
    \begin{figure}[H]   
        ...
        \end{figure}
\end{itemize}
Following the decomposition phase, the algorithm visits the resulting adjacent
cells, typically employing a Depth-First Search (DFS) strategy. To guarantee
...
testing, and validation sets with a ratio of 60:20:20 respectively. A total of 70,125 
...
loss curves are close to each other and decrease steadily. Further performance assessment results 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Included_Images/F1.png}
...
for which the true values are known. The diagonal elements of a confusion matrix represent correctly classified samples,
...
Rate (FPR) and helps to visualize the trade-off between sensitivity and specificity of the model. If the ROC value is
