...
Waypoint-based navigation can achieved in various approaches such as
...
vegetation environments, indentifying potential Fresnel zones, we primarily need
...
the robot to move. These positions depend with the gird architectures such as
4-way connected or 8-way connected grids. In addition to the conevtional grid
architecture, novel grid mapping approaches such as NavMeshes are widely used in
3D simulation softwares. These mapping techniques let the algorithm connects the
...
\begin{figure}[ht]
    ...
    \caption{Map Highlighting Shortest Path from Node A through Node K}
    \label{fig:astar}
\end{figure}
