...
into two key measurements: 1) the signal's carrier to noise ratio (CNR), and 2)
the excess pseudorange delay of the reflected signal after compensating for the
delay caused by geometrical reflection. These measurements are then analyzed in
correlation with the ground regions that the signals reflected off from - the
First Fresnel Zones (FFZ), while any correlation between GNSS measurements and
ground parameters will be modelled both quantitatively and qualitatively.
Ultimately, such models will be used to retrieve highly correlated ground
parameters from key GNSS signal measurements.
...
are comprehensively assessed and modeled.
