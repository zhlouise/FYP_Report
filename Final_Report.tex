...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Included_Images/remotesensing_schematic.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Included_Images/Spectral_camera_image.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Included_Images/GNSS-R schematic.png}
...
The GNSS-R framework or approach can characterize the reflected signals through
observable parameters such as the carrier to Noise Ratio ($C/N_0$), however the
relationship between measurements and the terrain type is complex and
non-linear. This complex relationship makes purely traditional and analytical
modelling approaches difficult to implement. Therefore, a machine learning based
approach can complement the GNSS-R framework to make more precise modelling of
the measurements. Recent studies have shown that machine learning models are
well suited to explore the multidimensional nature of GNSS-R measurements. A
recent study on neural network based approach for spaceborne GNSS-R data has
demonstrated to achieve high prediction results for Vegetation Water Content
(VWC) when compared to reference values \cite{chen2024new}. This shows the
potential to utilize Machine Learning based modelling for GNSS measurements in
different scenarios.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Included_Images/distibution1.png}
    \includegraphics[width=0.8\textwidth]{Included_Images/distibution2.png}
    \caption{Distribution of $C/N_0$ measurements collected with the downward-facing antenna (top) and upward-facing antenna (bottom).}
    \label{fig:cnr_distribution}
\end{figure}

In the context of signal classification and vegetation boundary detection,
Machine Learning remains an unexplored technique. Therefore, this project aims
to utilize the power of machine learning based modelling or classification for
the GNSS-R dataset. The raw GNSS signal features available from the dual antenna
setup constitutes a rich knowledge set that is suitable for training a machine
learning model for classifying signal types and surface characteristics. Figure
\ref{fig:cnr_distribution} shows the difference in characteristics of signals
collected with downward and upward facing antennas. As a result, this project will
incorporate a machine learning based approach to classify received signal type
and utilize it to detect vegetation boundary incorporation with the Fresnel Zone
Locations.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Data 6}
    \end{subfigure}
    \caption{GPS covariance comparison from different datasets.}
\end{figure}

By referring to the mean covariance from all datasets, covariance threshold is set to 0.1 to allow SLAM system to trust the GPS measurement while avoiding large GPS noises being included. Figure below shows the point clouds and trajectory from the pose graph SLAM system with GPS incorporated. 
...
For this dataset, LiDAR point clouds were processed using two techniques: the standard Fast-LIO odometry and PGO with GPS fusion (PGO+GPS). As shown in the figures below, both Fast-LIO and (PGO+GPS) are able to showcase point clouds of different features such as bare ground and tree canopies. In this case, the vegetation map generated can be used as a geometric reference for GNSS-R signals. In PGO+GPS, the green trajectory indicates the LiDAR odometry, and the 3-axis trajectory indicates the GPS trajectory. 
\begin{figure}[H]
    \centering
...
of the predictions align well with the visible terrain on the map. For example, satellites E21, E34 are classified as tree reflected
while their centroids also fall over the vegetated area predominantly.

The per satellite confidence plots are also shown below. They show that the classifier performs
...
strongly agree with the ground regions highlighted in the GNSS-R framework,
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Data 2}
    \end{subfigure}
    \caption{Inconsistencies when using fixed calibration information for frame transformation. Green trajectory indicates LiDAR trajectory while the 3-axis trajectory indicates the GPS trajectory.}
\end{figure}
...
However, this method has its own limitations. It introduces an inherent source of noise in the labeling because in practice, the nadir antenna may occasionally receive direct signals
even when the UAV is hovering at low altitude over bare ground or trees. The same might happen for the upward facing or the zenith antenna. Occasional signals reflected from the ground or trees
...
Furthermore, the class imbalance was another issue present in the dataset. The data collection scenarios for the tree cases were not equal. The distribution of the satellite geometry also
skewed the dataset for each class. A strategy to balance class weight was applied through the pre-processing of the data. Although it mitigates the imbalance to some extent, it does not
remove the skewness or the underlying distribution issue fully. This could make the model overly sensitive to the minority class samples. In the future, in order to mitigate the labelling issue
...
be to stabilize the UAV fully in hovering mode before beginning the data collection. 
