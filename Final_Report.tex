...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Included_Images/pathplanning_workflow.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Included_Images/env_obstacles.png}
...
LiDAR SLAM GNSS-Reflectometry (GNSS-R), and machine learning for vegetation
analysis. 
...
environments. Moreover, the interactive path planner system provides the optimal
detection area by clustering the Fresnel zones, which provides a suitable
sweeping distance for coverage path planning. As a result, an optimizined survey
mission can be planned in QGroundControl to fly the UAV over vegetation. 
...
In parallel, a modular LiDAR SLAM framework was developed to complement the
ground parameters retrieved from GNSS-R analysis. It is found that the pose graph optimization (PGO)
