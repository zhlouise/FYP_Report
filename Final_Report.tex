...
missions can be planned in QGroundControl to fly the UAV over vegetation. 

In parallel, a modular LiDAR SLAM framework was developed to complement the
ground parameters retrieved from GNSS-R analysis. It is found that the Pose
Graph Optimization (PGO) SLAM demonstrates better point cloud alignment compared
to Fast-LIO LiDAR odometry before the fusion of GPS data. The alignment between
SLAM and GPS trajectories were succesfully achieved by calibrating the IMU unit
in LiDAR and conducting LiDAR to GPS frame transformation.
...
ground-reflected, and tree-reflected signals. The classification results
perfectly cross-validate the results obtained from the GNSS reflected signals. 
...
collecting data and testing developed methodologies. During the second semester,
the team addressed the limitations and conducted final experiments to collect
more robust data and achieved the project objectives.  
...
environment and QGround Control. As a result, simulations were conducted to
verify locally developed A* algorithm in dynamic UAV environment. Moreover, an
interactive path planning system was developed in order to identify GNSS signal
reflected areas to obtain stronger data points. 
...
without GPS integration. The issues occured with the point cloud alignment were
resolved by calibrating the IMU unit in LiDAR and conducting LiDAR to GPS frame
transformation. 
...
As a result, the team was able to successfully integrate the four pillars of the
project and demonstrate meaningful results. The path planning framework was able
to provide optimal detection areas for the UAV to fly over vegetation. The LiDAR
SLAM framework was able to develop SLAM by sucessfully integrating the GPS data
into the SLAM system, follwing post graph optimization (PGO) SLAM. The GNSS-R
analysis was able to identify the ground parameters from the GNSS-R data and the
ML model was able to classify the signals into direct, ground-reflected, and
tree-reflected signals. These results were able to cross-validate the results
obtained from the GNSS-R analysis.

The Gantt chart below summarizes the project timeline for each individual parts
of the project.
...
though it was within the specified communication range. However, to solve this
problem, we have analyzed strong long-distance communication telemetry types and
will be replacing the current hardware in the next stage. 

Another limitation encountered is that not utilizing locally developed algorithms in
real-world experiments. This requires extensive testing on UAV indoor navigation
to ensure robust obtacle avoidance. However, due to the time limitations and
hardware restrictions, the real-world testing was not conducted.
