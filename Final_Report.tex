...
In order to incorporate GPS into the pose graph optimization SLAM, GPS measurements is treated as a constraint on a single node where error $\mathbf{e}_k$ for GPS measurements at time instant $k$ is obtained by taking the difference between the robot estimated position $\mathbf{p}_k$ and the GPS measurement $\mathbf{z}^{\mathrm{gps}}_k$:
\begin{equation}
\mathbf{e}_k = \mathbf{z}^{\mathrm{gps}}_k - \mathbf{p}_k
\end{equation}

Where $\mathbf{x}_k$ is the status of the robot at time $k$ and $\mathbf{z}^{\mathrm{gps}}_k$ is the GPS position measurement in the local frame. The error $\mathbf{e}_k$ is then used to obtain the cost function $F(X)$ of the pose graph optimization system which evaluates the total error score between the LiDAR trajectory and the GPS trajectory: 
\begin{equation}
