...
the front-end and GPS-coupled pose graph optimization SLAM as the back-end. As a
...
front-end and pose graph optimization as the back-end. As a result, the 3D
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Included_Images/EKF SLAM.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Included_Images/pgo.png}
...
This project investigates how such SLAM methods can be adapted and deployed for
real-time forestry applications.

\subsection{Methodology}
This section describes the methodology adopted to develop the proposed GPS-aided modular SLAM system for UAV remote sensing with the aim to reduce trajectory drift. To achieve this, the methodology is divided into several key components. First, Fast-LIO is used as the front-end odometry to fuse LiDAR and IMU data using an Iterative Extended Kalman Filter (IEKF) for real-time pose estimation and local map generation. Second, back-end optimization SLAM is employed to carry out trajectory refinement and loop closure detection. Third, GPS measurements are incorporated as absolute positioning factors to prevent long-term drift. Fourth, since LiDAR and GPS measurements originate from different sensor frames, frame transformation is carried out to align both data sources into the same coordinate system. Lastly, the complete system architecture is outlined to illustrate how the sensing, front-end estimation, and back-end optimization modules are interacted within the Robotic Operating System (ROS).
...
where $f_k(x_{k-1}, u_k)$ indicates the relative motion estimate from the front-end LiDAR odometry and $\Sigma_{u,k}$ represents its covariance. Similarly for the observation factor, which correspond to the GPS measurements can be written as:
\begin{equation}
...
\begin{figure}[H]
    ...
    \label{fig:stg2_pgo}
\end{figure}
Once GPS is being integrated to the PGO SLAM, we can see in
\autoref{fig:stg2_pgo} that there are two different trajectories and the point
clouds are very misaligned. Firstly, both trajectories are not aligned as the
position of the LiDAR and the GPS on the drone itself are different, hence
having two different frames. Second of all, the point clouds are misaligned as
GPS has a high gain in the SLAM and some issues related to the transformation of
the GPS to LiDAR odometry. 
...
% --- Case Study ---
\section{Case Study: 2026 March 31, Mount Davis Experiment}
In this case study, the results of Experiment (i) in Table
\ref{Experiment-Summary} are discussed to demonstrate the current integrated
system. Here, a survey flight mission was conducted to collect feature-extensive
LiDAR and GNSS-R datasets above a canopy area consisted of mixed dense
vegetation. 
...
signals calculated, they were plotted onto the satellite image map. Here, the color
of the FFZs indicates the $C/N_0$ ratio between the bottom and top antennas,
and strong reflections over water and bare ground could be observed compared to those
reflected from vegetation.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Included_Images/KeyExperiment_GNSSR.png}
