...
task will help to identify tree reflected signals which will be sent to GNSS-R framework, while the regression task will help to predict vegetation parameters and validate GNSS-R framework. Classification part will be mainly based on gradient boosting and
...
The report begins with a section on system architecture. It is followed by four core sections covering UAV path planning, LiDAR SLAM, GNSS-R, and machine learning respectively. Within each of these four sections includes literature review, methodology, results, and discussion to clearly present the work completed in each area. The report then concludes with sections on experiments conducted, case study of selected experiments, conclusion, and project management.
...
\subsubsection{GPS Correction}
Since LiDAR based SLAM system relies on relative measurements to estimate motion, this method is prone to trajectory drift if errors such as inaccuracy in pose incrementation and accumulation of non-collective uncertainty occurs during the computation \cite{cwian2021large, chen2022slam}. 

To counter this issue, GPS is added to the pose graph optimization SLAM system to provide absolute global positioning by detecting receiver’s location relative to a known Earth-centered reference coordinate system from satellite signals. The presence of GPS will then acts as an anchor to avoid the LiDAR SLAM from drifting due to relative positioning errors.

For the purpose of incorporating GPS into the pose graph optimization SLAM, GPS measurements is treated as a constraint on a single node where error $\mathbf{h}_k$ for GPS measurements at time instant $k$ is obtained by taking the difference between the robot estimated position $\mathbf{p}_k$ and the GPS measurement $\mathbf{z}^{\mathrm{gps}}_k$:
\begin{equation}
\mathbf{h}_k(x_k) = \mathbf{z}^{\mathrm{gps}}_k - \mathbf{p}_k
\end{equation}

Where $x_k$ is the status of the robot at time $k$ and $\mathbf{z}^{\mathrm{gps}}_k$ is the GPS position measurement in the local frame. The error $\mathbf{h}_k$ is then used to obtain the cost function $F(X)$ of the pose graph optimization system which evaluates how well the estimated robot trajectory agrees with botht the relative LiDAR constraints and the absolute GPS measurements: 
\begin{equation}
F(X)=\sum_i \left\| h_{i,i+1} \right\|^2_{\Omega_{odom}}
\;+\;
\sum_k \left\| z_k^{gps} - p_k \right\|^2_{\Omega_{gps}}
\end{equation}

Letting $\mathbf{e}_{i,i+1}$ denote the error between the LiDAR measurement and the robot's estimation. Meanwhile, $\Omega_{odom}$ is the information matrix representing the uncertainty of LiDAR odometry and $\Omega_{gps}$ is the information matrix of GPS uncertainty. These matrices determine how much influence each measurement source has in the optimization process based on its level of uncertainty. Measurements with lower uncertainty are given higher influence during optimization, while noisier measurements contribute less to the final solution. 

Since the total cost function $F(X)$ is generally nonlinear with respect to the robot states, iterative nonlinear least-squares methods are used to solve this problem. Based on that, Levenberg-Marquardt algorithm is a standard optimization method used to iteratively solve for the trajectory update using the Jacobian matrix \cite{ranganathan2004levenberg}. In each iteration, the nonlinear error function is linearized around the current state estimate and an increment $\Delta X$ is computed to update the pose estimate until convergence is achieved. The state update $\Delta X$ can be expressed as:
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Illustration of the direction of principal axis within the SLAM system}
    \label{fig:SLAM_axis}
\end{figure}

Meanwhile, as GPS only provide translational data such as longitude, latitude, and altitude, additional IMU which is located at the same position as the GPS antenna, is used to provide the rotational information for GPS measurement, creating a complete 6-DoF pose representation required for accurate frame transformation. Based on that, extrinsic calibration is carried out between the GPS/IMU system and LiDAR/IMU to obtain the relative rotation between two sensor frames. The calibration results indicate that there is a rotational offset of $145.98^\circ$ about the X-axis, $90.0^\circ$ about the Z-axis, and no rotational offset about the Y-axis between the GPS and LiDAR frames. 
...
To bridge this gap, initial IMU's global orientation from the GPS is captured at system startup representing as a quaternion $Q_{imu_global}$. Even so, given that GPS/IMU is rigidly attached to the LiDAR sensor via a known extrinsic calibration, its orientation must be corrected by the previously determined fixed transformation between the IMU and LiDAR frames where the transformation is represented as $Q_{imu_to_lidar}$.
